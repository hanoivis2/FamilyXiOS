...
class AddPeopleViewController : UIViewController, NavigationControllerCustomDelegate {
    ...
    var allGenderType: [GenderType] = [GenderType(id: 0, text: "Female"),
                                        GenderType(id: 1, text: "Male")]
    
    var allRelationshipType: [RelationshipType] = [RelationshipType(id: 0, text: "Husband / Wife"),
    ...
    @IBAction func btn_confirm(_ sender: Any) {
        delegate?.addPeople(name: textfield_name.text!, birthday: textfield_birthday.text!, gender: genderSelected, phone: textfield_phone.text!, avatar: img_avatar.image!)
        navigationController?.popViewController(animated: true)
    }
    ...
    @IBAction func btn_choose_birthday(_ sender: Any) {
        ...
        datePicker.show("Choose your birthday", doneButtonTitle: "Done", cancelButtonTitle: "Cancel", defaultDate: Date(), minimumDate: Calendar.current.date(byAdding: dateComponents, to: Date()), maximumDate: Calendar.current.date(byAdding: dateComponentsFuture, to: Date()), datePickerMode: .date) { (date) in
            if let dt = date {
